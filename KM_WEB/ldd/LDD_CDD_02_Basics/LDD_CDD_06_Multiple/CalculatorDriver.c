...
struct cdev *my_cdev;
struct semaphore my_sem;  //1.) creation of semaphore object
...
*/	
	...
	sema_init(&my_sem,1); //2.) initialize semaphore variable with 1 means lock the door
	return 0;
}
...
int NAME_open(struct inode *inode, struct file *filp)
{
	...
	down(&my_sem);//3.) lock the door by decrementing sema value to 0 , or wait if previously locked
	switch (Minor) {
...
int NAME_release(struct inode *indoe, struct file *filp)
{
	...
	up(&my_sem); //4.) unlock the door, by again making semaphore variable to 1
	return 0;
} 
